...
class FeedForward(nn.Module):
    ...
    No activation is applied at the output of ViT-VQGAN encoder or decoder
    """
...
class Attention(nn.Module):
    """
    ViT-VQGAN is designed to be without dropout according to Table 2 in the paper.
    """
...
class ViTDecoderForCls(nn.Module):
    def __init__(self, image_size: Union[Tuple[int, int], int], patch_size: Union[Tuple[int, int], int],
                 dim: int, depth: int, heads: int, mlp_dim: int, channels: int = 3, dim_head: int = 64, num_classes: int=1000) -> None:
        super().__init__()
        image_height, image_width = image_size if isinstance(image_size, tuple) \
                                    else (image_size, image_size)
        patch_height, patch_width = patch_size if isinstance(patch_size, tuple) \
